...
test.describe(
  ...
  () => {
    ...
    test(
      'CART-002: adds multiple products to the cart',
      async ({
        inventoryPage,
        cartPage,
      }) => {
    ...
    test(
      'CART-003: removes a product from the inventory page',
      async ({ inventoryPage }) => {
        const product = cartProducts.backpack;

        const productCard =
          inventoryPage.getProductCard(
            product.name,
          );
    ...
    test(
      'CART-004: removes a product from the cart page',
      async ({
        inventoryPage,
        cartPage,
      }) => {
    ...
    test(
      'CART-005: displays the correct product details in the cart',
      async ({
        inventoryPage,
        cartPage,
      }) => {
        ...
        const cartItem = cartPage.getCartItem(
          product.name,
        );

        await expect(
          cartItem.getByTestId(
            'inventory-item-name',
          ),
    ...
    test(
      'CART-006: continues shopping without losing cart contents',
      async ({
        page,
        inventoryPage,
        cartPage,
